...
\[
E(\textrm{length of smaller piece}) = \frac{1}{2}\cdot\frac{1}{2}=\frac{1}{4}\,.
\]

\ans{2} Without loss of generality assume that the break occurred in the right half of the rod (Figure~\ref{f.stick}). The ratio of the smaller piece to the larger piece is $(1-x)/x$ and the length of the larger piece $x$ is uniformly distributed in $(1/2,1)$. The expectation of the ratio is:
\begin{eqnarray*}
E(\textrm{ratio})&=&\left(\frac{1}{1-(1/2)}\right)\int_{1/2}^1 \frac{1-x}{x} \,dx\\
...
\begin{prob}{המוט השבור}{S}{(The broken bar)}

A large number of glass rods of length $1$ are broken in two places (Figure~\ref{f.break1}).

\que{1} What is the expectaton of the length of the shortest bar?

\que{2} What is the expectaton of the length of the longest bar?
...
\selectlanguage{hebrew}
\subcaptionbox{%
שטח מוצלל עבור המקל הקצר ביותר%
\label{f.shaded1}}
[.45\textwidth]
{
\centering
...
\hspace{1em}
\subcaptionbox{%
שטח מוצלל עבור המקל הארוך ביותר%
\label{f.shaded2}}
[.45\textwidth]
{
\centering
...
\begin{eqnarray*}
E(x \textrm{\ in left triangle})&=& 6\int_{1/3}^{1/2} x [2x-(1-x)]\,dx  \\
&=&\int_{1/3}^{1/2} \left(18x^2-6x\right)\,dx\\
&=&\left. (6x^3-3x^2)\right|_{1/3}^{1/2}=\disfrac{1}{9}\\
E(x \textrm{\ in right triangle})&=& 6\int_{1/2}^{1} x [1-x]\,dx\\
...
\que{1} Develop a formula for the probability $P_N$ of winning the game and develop a formula for the probability $T_N$ of a tie occurring.
...
\ans{1} To win the game head needs to appear in $i\in\{n+1, n+2, \ldots, 2n-1, 2n=N\}$ tosses. From the binomial distribution:
\begin{eqnarray*}
...
\[
P_{2n-2} \leq P_{2n} \quad \textrm{and} \quad P_{2n}\geq P_{2n+2}\,.
\]
When is $P_{2n-2}\not = P_{2n}$?
...
\[
E(\textrm{number of matches}) = 52\cdot \frac{1}{52} = 1\,.
\]
...
\begin{prob}{לבחור את המוהר הגדול ביותר}{D,S}{(Choosing the largest dowry)}
...
\que{1} For $n=4$ and $r=3$ check all permutations to determine how many games you will win.
...
\[
1\quad 2\quad \cdots\quad r-2 \quad r-1 \quad \overbrace{r \quad r+1 \quad \cdots\quad m-1\quad  m \quad m+1\quad \cdots \quad n}^{\textrm{largest number must be here}}\,.
\]
...
\[
P(\textrm{largest card in}\;(1,\ldots,m-1)\;\textrm{is in}\; (1,\ldots,r-1)) = \disfrac{r-1}{m-1}\,.
\]
Since the probability that the largest card is at $m$ is $1/n$:
\begin{equation}\label{eq.dowry1}
P(\textrm{win}) = \sum_{m=r}^{n} \disfrac{1}{n} \cdot \disfrac{r-1}{m-1}= \disfrac{r-1}{n}\sum_{m=r}^{n} \disfrac{1}{m-1}\,.
\end{equation}
For $n=4, r=3$, $P(\textrm{win}) = 5/12=10/24$.
...
P(\textrm{win}) =\disfrac{r-1}{n}\left(\sum_{m=2}^{n} \disfrac{1}{m-1}-\sum_{m=2}^{r-1} \disfrac{1}{m-1}\right)\,.
\end{equation}
For large $n,r$ Equation~\ref{eq.dowry2} can be approximated by:
\[
P(\textrm{win})=\disfrac{r}{n}(\ln n - \ln r)=\disfrac{r}{n}\ln \disfrac{n}{r}=-\disfrac{r}{n}\ln \disfrac{r}{n}\,.
\]
...
\ans{2} If you select the first card $P(\textrm{win})=v_1$ which is the probability that the second card has a smaller value. If you reject the first card, $P(\textrm{win})=1-v_1$, which is the probability that $v_2>v_1$. Therefore, if $v_1<0.5$ select the second card because $1-v_1>0.5$ and if $v_1>0.5$ select the first card because $1-v_1<0.5$. It follows that $d=0.5$.

Here is the formula for the probability of winning:
\[
P(\textrm{win with two cards}) = p(\textrm{win} \,|\,v_1<0.5)\,p(v_1<0.5)+ p(\textrm{win}\,|\,v_1>0.5)\,p(v_1>0.5)\,.
\]
$p(v_1<0.5)=0.5$ is immediate by uniform distribution. What about $p(\textrm{win} \,|\,v_1<0.5)$? By the strategy you win if $0.5<v_2<1$, but you also win if $v_1<v_2<0.5$. Since $v_1$ is uniformly distributed in $(0,0.5)$:
\[
p(\textrm{win} \,|\,v_1<0.5)=\disfrac{1}{2} + \disfrac{1}{4}=\disfrac{3}{4}\,.
\]
A similar computation holds for $v_1>0.5$. Putting this together gives:
\[
P(\textrm{win with two cards})=\disfrac{3}{4}\cdot\disfrac{1}{2}+\disfrac{3}{4}\cdot\disfrac{1}{2}=\disfrac{3}{4}\,.
\]

\ans{3}

If you select the first card $P(\textrm{win})=v_1^2$ because the second and third cards must be smaller than the first.

If you reject the first card and select the second because $v_2>v_1$:
\begin{itemize}
\item $P(\textrm{win})=(1-v_1)v_1$ if $v_2>v_1$ and $v_3<v_1$.
\item $P(\textrm{win})=v_1(1-v_1)$ if $v_2<v_1$ and $v_3>v_1$.
\item $P(\textrm{win})=\frac{1}{2}(1-v_1)^2$ if $v_2>v_1$ and $v_3>v_1$, since winning depends on the order: $(0.55, 0.75, 0.65)$ wins and $(0.55, 0.65, 0.75)$ loses.
\end{itemize}
...
\[
P(\textrm{win}) = \disfrac{1}{3}+\disfrac{d}{2}+\disfrac{d^2}{1}-\disfrac{3d^3}{2}\approx 0.6617\,.
\]
\textbf{Simulation:}
\begin{verbatim}
For  3 cards:
