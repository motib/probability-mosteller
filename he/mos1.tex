...
פתרון זה מיוחס לפסקל ומבוסס על 
\cite[סעיף~20.1.1]{mit}.

\ans{2}
במקום צעדים על ציר, שני המהמרים מתחילים על ערימות של אסימונים: למהמר 
$A$
יש
$m$
אסימונים ולמהמר
...
יש
$n-m$
אסימונים. האסימונים לא זהים: אסימון
$i$
מסומן 
$r^i$
יחידות, כאשר 
$r$
הוא
$(1-p)/p$ (איור~\ref{f.pascal1}).
אנחנו מניחים ש-%
$1/2<p<1$
ולכן
$r^i>r^{i+1}$,
כפי שמסומן ברוחב האסימונים באיור.

אם
$A$
מנצח (בהסתברות
$p$)
הוא לוקח את האסימון העליון מ-%
$B$
ושם אותו בראש הערימה שלו. אם
$B$
מנצח (בהסתברות
$1-p$)
היא לוקחת את האסימון העליון מ-%
$A$
ושמה אותו בראש הערימה שלה. באיור~%
\ref{f.pascal2} 
אם
$A$
מנצח הוא מוסיף
$r^{5}$
יחידות לערימה שלו, בעוד עם 
$B$
מנצחת היא מוסיפה
$r^4$
יחידות לערימה שלה. סכום היחידות של האסימונים בערימה אחת הוא כמות הכסף הנוכחית של המהמר. עבור סיבוב אחד של המשחק:
\begin{eqnarray*}
E(\textrm{יחידות} \:i\:\textrm{\R{רווח של A אם יש לו}})&=&pr^{i+1}-(1-p)r^i\\
&=&r^i\left(\frac{p(1-p)}{p}-(1-p)\right)\\
&=&0\,.
\end{eqnarray*}
התנאים של כל סיבוב זהים ולכן תוחלת הרווח של 
$A$
בסוף המשחק היא גם כן
$0$.
תהי 
$P_A$
ההסתברות ש-%
$A$
ינצח בסוף אם הוא מתחיל מ-%
$m$
יחידות. הנוסחה שלהלן חייבת להיות שווה ל-%
$0$:
\[
...
$1/2<p<1$,
$r\neq 1$,
ולכן לפי הנוסחה לסכום של סדרה הנדסית:
\[
...
\end{scope}
\end{tikzpicture}
\caption{ל-$A$ יש $5$ אסימונים ול-$B$ יש $4$ אסימונים}
\label{f.pascal1}
\end{center}
\end{figure}
...
\end{scope}
\end{tikzpicture}
\caption{ערימות האסימונים לאחר נצחון של $B$}\label{f.pascal2}
\end{center}
\end{figure}
