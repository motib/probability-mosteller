...
\que{1} How many people do you have to ask until the probability of finding your brithmate is greater than $0.5$?
...
\[
P(\textrm{not a birthmate})=\left(\frac{364}{365}\right)^k<\frac{1}{2}\,,
\]
which is $k=253$:
\[
...
Let $P_{\textrm{\footnotesize pair}}(r)$ be the probability that two people out of $r$ are a birthday pair (Problem~$31$) and let $P_{\textrm{\footnotesize mate}}(n)$ be the probability that two people out of $n$ are birthmates (Problem~$32$). Given $r$ for what $n$ does $P_{\textrm{\footnotesize pair}}(r) \approx P_{\textrm{\footnotesize mate}}(n)$?
\end{prob}
...
Using the notation $P_{\textrm{\footnotesize no pair}}(r)$ for the complement, from the solution to Problem~$31$ we have:
\[
\renewcommand*{\arraystretch}{2.2}
\begin{array}{lcl}
P_{\textrm{\footnotesize no pair}}(r)&=&
...
Using the notation $P_{\textrm{\footnotesize no mate}}(n)$ for the complement and the same approximation, from the solution to Problem~$32$ we have:
\[
\renewcommand*{\arraystretch}{2.2}
\begin{array}{lcl}
P_{\textrm{\footnotesize no mate}}(n)
...
\end{array}
\]
Therefore $P_{\textrm{\footnotesize no pair}}(r)\approx P_{\textrm{\footnotesize no mate}}(n)$ when:
\[
n=\frac{r(r-1)}{2}\,.
\]
...
\]
ways of doing so. For the birthmate problem your own birthday is given. Any of the $n$ people can have the same birthday. By equating the two we have the $n$ such that $P_{\textrm{\footnotesize pair}}(r) \approx P_{\textrm{\footnotesize mate}}(n)$.
...
\que{1} How many workers should be employed in order to maximize the number of work-days in a year?
...
\[
P(\textrm{a given day is a work-day})=\overbrace{\disfrac{N-1}{N} \cdot \;\cdots\;\cdot \disfrac{N-1}{N}}^n = \left(1-\disfrac{1}{N}\right)^n\,.
\]
Denote $\left(1-\disfrac{1}{N}\right)$ by $p$.

The expectation of the number of work-days for a given day is:
\[
E(\textrm{work-days for a given day}) = n \cdot p^n + 0\cdot (1-p^n) = np^n\,.
\]
...
E(\textrm{work-days for a year}) = Nnp^n\,.
\end{equation}
...
\]
%$(a^x)'=a^x\ln a$ which can be proved using the chain rule:
%\[
%(a^x)' = ((e^{\ln a})^x)' =
%(e^{x\ln a})' =
%e^{x\ln a} (x\ln a)'=
...
\[
E(\textrm{max work-days/possible work-days})=\disfrac{365\cdot 365  \cdot \left(\frac{364}{365}\right)^{365}}{365\cdot 365}=\left(\disfrac{364}{365}\right)^{365}\approx 0.3674\,.
\]
By Equation~\ref{eq.reciprocal}:
\[
\lim_{n\rightarrow\infty}E(\textrm{max work-days/possible work-days})=\lim_{N\rightarrow \infty} \left(1-\disfrac{1}{N}\right)= \disfrac{1}{e}\,.
\]
...
\begin{eqnarray*}
P(\textrm{reaches}\;0\;\textrm{from}\;1) &=& P(\textrm{reaches}\;0\;\textrm{from}\;1|\textrm{first move left}) +\\
&&P(\textrm{reaches}\;0\;\textrm{from}\;1|\textrm{first move right})\\
&=& (1-p)\cdot 1 + pP(\textrm{reaches}\;1\;\textrm{from}\;2)P(\textrm{reaches}\;0\;\textrm{from}\;1)\,.
\end{eqnarray*}
But the probability of reaching $1$ from $2$ is exactly the same as the probability of reaching $0$ from $1$. Abbreviating $P(\textrm{reaches}\;0\;\textrm{from}\;1)$ as $P$ we have:
\begin{eqnarray*}
P &=& (1-p) + pP^2\\
pP^2 - P + (1-p) &=&0\\
...
\que{2} Let $n>m$. If the particle reaches position $0$ or position $n$ its stops moving. What is the probability that the particle will eventually be at position $0$? What is the probability that the particle will eventually be at position $n$?
\begin{figure}[tb]
\begin{center}
\begin{tikzpicture}[scale=1.5]
\draw (0,0) -- (6,0);
...
\ans{1} The solution to Problem~35 showed that for $p>1/2$ (which is true here by assumption), if a particle is at position $1$ the probability of its reaching position $0$ is $r=(1-p)/p$. Notation: let $P(i,j)$ be the probability of reaching $i$ from $j$. Since the claim does not depend on the actual position of the particle, the probability of a particle reaching $0$ from position $m$ is:
\begin{equation}
...
\[
P(\textrm{reaches}\;40\;\textrm{from}\;20) =
