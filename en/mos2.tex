...
\ans{2} Without loss of generality assume that the break occurred in the right half of the rod (Figure~\ref{f.stick}). The ratio of the smaller piece to the larger piece is $(1-x)/x$ and a normalization constant (page~\pageref{p.normal}) must be used because we want the expectation over half the range of $x$:
\begin{eqn}
...
\end{equation}
$p(v_1<0.5)=0.5$ follows by the uniform distribution. What about $p(\textsf{win} \,|\,v_1<0.5)$? By the strategy you win if $0.5<v_2<1$, but you also win if $v_1<v_2<0.5$. Since $v_1$ is uniformly distributed in $(0,0.5)$ the probability that $v_2$ is larger is one-half the range:
\[
...
\begin{itemize}
\item $P(\textsf{win})=v_1(1-v_1)$ if $v_2<v_1$ (reject $v_2$) and $v_3>v_1>v_2$ (select $v_3$ and win).
\item $P(\textsf{win})=(1-v_1)v_1$ if $v_2>v_1$ (select $v_2$) and $v_3<v_1$ (win).
\item $P(\textsf{win})=\frac{1}{2}(1-v_1)^2$ if $v_2>v_1$ and $v_3>v_1$. The factor of $1/2$ takes into account that winning depends on whether $v_3<v_2$ (win) or $v_2<v_3$ (lose).
\end{itemize}
...
\que{3} Use Stirling's approximation to estimate the probability for large $n$.
...
\begin{eqnlabels}
\nonumber{}P_{2n}(\textsf{return to}\;x\!=\!0) =P_{2n}(\textsf{return to}\;y\!=\!0)&=&
...
\label{eq.rw-2d}P(\textsf{return to origin}) &=& \disfrac{1}{\pi}\sum_{n=1}^{\infty}\disfrac{1}{n}\,,
\end{eqnlabels}%
...
\label{eq.rw-3d}P_r &=& \sum_{n=1}^{\infty}\disfrac{1}{(\pi n)^{3/2}}\approx 0.3772\,.
\end{eqnlabels}%
...
\textbf{Higher dimensions:} Equation~\ref{eq.rw-multiply} can be extended to any number of dimensions and from Equations~\ref{eq.rw-2d}, \ref{eq.rw-3d}, it is reasonable to conjecture that $P_r$ is \emph{proportional to}:
\begin{equation}
\label{eq.rw-sum}
\sum_{n=1}^{\infty} \disfrac{1}{n^{d/2}}\,,
\end{equation}
where $d$ is the dimension \cite{louigi}. Now use the \emph{Cauchy condensation test} \cite{wiki:cauchy}, which for the sum in Equation~\ref{eq.rw-sum} is:
\[
...
\]
For $d=2$ the result is $\sum_{n=1}^{\infty} 1$ which clearly diverges. For $d=3$ the sum is:
\[
\sum_{n=1}^{\infty} \disfrac{1}{n^{3/2}}=\sum_{n=1}^{\infty} \disfrac{2^n}{(2^n)^{3/2}}=\sum_{n=1}^{\infty} \disfrac{2^n}{2^n\cdot 2^{n/2}}=\sum_{n=1}^{\infty} \disfrac{1}{(\sqrt{2})^n}=\frac{\sqrt{2}}{1-\sqrt{2}}\,.
\]
For $d=4$ the sum is $\sum_{n=1}^{\infty} \frac{1}{2^n}=2$ and for higher dimensions the probability is finite although it decreases to very small values.
...
Let $U_1,U_2$ be two urns. $U_1$ has $w_1$ white balls and $b_1$ black balls, while $U_2$ has $w_2$ white balls and $b_2$ black balls. It is given that $w_1+b_1=w_2+b_2$. $n$ balls are drawn \emph{with replacement} from each urn.

\que{1} For various values of $n>1$ find $w_1,b_1,w_2,b_2$ such that:
\[
