...
Given an unfair coin whose probability of heads is $1/3 < p < 1/2$, toss a coin an even number of times $N=2n$. You win if and only \emph{more than half} of the tosses are heads.

\que{1} Develop a formula for $P_N$, the probability of winning, and develop a formula for $T_N$, the probability of a tie.
...
\ans{1} To win, heads needs to appear in $i\in\{n+1, n+2, \ldots, 2n-1, 2n\}$ tosses. From the binomial distribution:
\[
