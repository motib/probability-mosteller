...
\end{eqnlabels}
From Equations~\ref{eq.ruin0}, \ref{eq.ruin00}, for any initial position $m$:
\begin{equation}
\label{eq.ruin1}P(n,m) = \left(\frac{1-r^{m}}{1-r^n}\right)\,.
\end{equation}
Using a symmetrical argument exchanging $p$ and $1-p$:
\begin{equation}
\label{eq.ruin2}P(0,m) = \left(\frac{1-(1/r)^{n-m}}{1-(1/r)^{n}}\right)\,.
\end{equation}
...
The solution is based on \cite[Section~20.1.1]{mit} where it is attributed to Pascal.

\ans{2}
Instead of steps on an axis, the two gamblers start out with piles of chips: gambler $A$ has $m$ chips and gambler $B$ has $n-m$ chips. The chips are not identical: chip $i$ is labeled with $r^i$ units, where $r$ is $(1-p)/p$ (Figure~\ref{f.pascal1}). Since we are assuming $1/2<p<1$, $r^i>r^{i+1}$ as indicated by the width of the chips in the figure.

If $A$ wins (with probability $p$) he takes the top chip from $B$ and places it on the top of his pile. If $B$ wins (with probability $1-p$) she takes the top chip from $A$ and places it on the top of her pile. In  Figure~\ref{f.pascal2} if $A$ wins he adds $r^{5}$ units to his pile while if $B$ wins she adds $r^4$ units to her pile. The sum of the units of the chips in a pile is that gambler's current capital. For one round of the game:
\begin{eqnarray*}
E(A \textsf{'s winnings if\ }A  \textsf{\ has\ }i \textsf{\ units})&=&pr^{i+1}-(1-p)r^i\\
&=&r^i\left(\frac{p(1-p)}{p}-(1-p)\right)\\
&=&0\,.
\end{eqnarray*}
Since the conditions of all rounds are the same, $A$'s expected winnings at the end of the game are also $0$. Let $P_A$ be the probability that $A$ wins at the end if he starts with $m$ units; the following formula must equal $0$:
\[
P_A\sum_{j=m+1}^{n}r^j - (1-P_A)\sum_{j=1}^{m}r^j=0\,.
\]
Since we assume that $1/2<p<1$, so $r\neq 1$, the formula for the sum of a geometric series results in:
\[
P_A \frac{r^{m+1} - r^{n+1}}{1-r} - (1-P_A) \frac{r-r^{m+1}}{1-r}=0\,,
\]
which simplifies to:
\[
P_A =  \left(\frac{1-r^{m}}{1-r^n}\right)\,.
\]

\begin{figure}
\begin{center}
\begin{tikzpicture}[every rectangle node/.style={rectangle,minimum height=.6cm}]
\node[draw,minimum width=5cm] at (0,0) {$r^1$};
\node[draw,minimum width=4.5cm] at (0,.6) {$r^2$};
\node[draw,minimum width=4cm] at (0,.6*2) {$r^3$};
\node[draw,minimum width=3.5cm] at (0,.6*3) {$r^4$};
\node[draw,minimum width=3cm] at (0,.6*4) {$r^5$};
\node at (0,-.7) {$A$};
\begin{scope}[xshift=5cm]
\node[draw,minimum width=2.5cm] at (0,.6*3) {$r^6$};
\node[draw,minimum width=2cm] at (0,.6*2) {$r^7$};
\node[draw,minimum width=1.5cm] at (0,.6*1) {$r^8$};
\node[draw,minimum width=1cm] at (0,.6*0) {$r^9$};
\node at (0,-.7) {$B$};
\end{scope}
\end{tikzpicture}
\caption{$A$ has $5$ chips and $B$ has $4$ chips}\label{f.pascal1}
\end{center}
\end{figure}
...
\begin{center}
\begin{tikzpicture}[every rectangle node/.style={rectangle,minimum height=.6cm}]
\node[draw,minimum width=5cm] at (0,0) {$r$};
\node[draw,minimum width=4.5cm] at (0,.6) {$r^2$};
\node[draw,minimum width=4cm] at (0,.6*2) {$r^3$};
\node[draw,minimum width=3.5cm] at (0,.6*3) {$r^4$};
\node at (0,-.7) {$A$};
...
\end{scope}
\end{tikzpicture}
\caption{Piles of chips after $B$ wins}\label{f.pascal2}
\end{center}
\end{figure}
